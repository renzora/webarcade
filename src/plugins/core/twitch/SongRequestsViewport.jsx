...
export default function SongRequestsViewport() {
  ...
  const loadRequests = async () => {
    try {
      const pendingResponse = await bridgeFetch('/song-requests/pending');
      ...
      if (showHistory()) {
        const allResponse = await bridgeFetch('/song-requests/all?limit=50');
  ...
  const updateStatus = async (id, status) => {
    try {
      const response = await bridgeFetch('/song-requests/status', {
        method: 'POST',
  ...
  const deleteRequest = async (id) => {
    ...
    try {
      const response = await bridgeFetch('/song-requests/:id', {
        method: 'DELETE',
        headers: { 'Content-Type': 'application/json' },
  ...
  const clearCompleted = async () => {
    ...
    try {
      const response = await bridgeFetch('/song-requests/clear', {
        method: 'DELETE',
        headers: { 'Content-Type': 'application/json' },
