...
pub async fn register_routes(ctx: &PluginContext) -> Result<()> {
    ...
    // GET /roulette/enabled?channel=xxx - Check if roulette is enabled
    router.route(Method::GET, "/enabled", |_path, query, _req| {
        Box::pin(async move {
            handle_get_enabled(query).await
        })
    });

    // POST /roulette/toggle - Toggle roulette on/off
    router.route(Method::POST, "/toggle", |_path, _query, req| {
        Box::pin(async move {
            handle_toggle_enabled(req).await
        })
    });
    ...
    router.route(Method::OPTIONS, "/toggle", |_path, _query, _req| {
        Box::pin(async move { cors_preflight_response() })
    });
...
async fn handle_get_enabled(query: String) -> Response<BoxBody<Bytes, Infallible>> {
    let channel = match parse_query_param(&query, "channel") {
        Some(ch) => ch,
        None => return error_response(StatusCode::BAD_REQUEST, "Missing channel parameter"),
    };
...
async fn handle_toggle_enabled(req: Request<Incoming>) -> Response<BoxBody<Bytes, Infallible>> {
    match read_json_body(req).await {
        Ok(body) => {
            let channel = match body.get("channel").and_then(|v| v.as_str()) {
                Some(ch) => ch,
            ...
            let enabled = match body.get("enabled").and_then(|v| v.as_bool()) {
                Some(e) => e,
                None => return error_response(StatusCode::BAD_REQUEST, "Missing or invalid enabled value"),
            };
            ...
            match rusqlite::Connection::open(&conn) {
                Ok(conn) => {
                    ...
                    match conn.execute(
                        "INSERT OR REPLACE INTO roulette_config (channel, enabled, updated_at) VALUES (?1, ?2, ?3)",
                        rusqlite::params![channel, if enabled { 1 } else { 0 }, now],
                    ) {
                        Ok(_) => {
                            json_response(&serde_json::json!({
                                "success": true,
