...
function RouletteOverlay() {
    ...
    ws.onmessage = (event) => {
      try {
        ...
        if (data.event_type && data.payload) {
          ...
          if (eventType === 'roulette.enabled') {
            console.log('[Roulette] Roulette enabled!');
            setIsEnabled(true);
            loadCurrentGame();
          } else if (eventType === 'roulette.disabled') {
            console.log('[Roulette] Roulette disabled!');
            setIsEnabled(false);
            setShowWheel(false);
            setCurrentGame(null);
            stopBetTimer();
          ...
          } else if (eventType === 'roulette.game_started') {
  ...
  const loadEnabledState = async () => {
    ...
    try {
      const response = await fetch(`${BRIDGE_API}/roulette/enabled?channel=${channel()}`);
      const data = await response.json();
      setIsEnabled(data.enabled);
      console.log('[Roulette] Enabled state loaded:', data.enabled);
    } catch (error) {
      console.error('[Roulette] Failed to load enabled state:', error);
  ...
  const loadCurrentGame = async () => {
    ...
    // Don't show game if roulette is disabled
    if (!isEnabled()) {
      console.log('[Roulette] Roulette is disabled, not loading game');
      return;
    }

    try {
      const response = await fetch(`${BRIDGE_API}/roulette/game?channel=${channel()}`);
      const data = await response.json();
  ...
  const loadHistory = async () => {
    ...
    try {
      const response = await fetch(`${BRIDGE_API}/roulette/history?channel=${channel()}&limit=5`);
      const data = await response.json();
      setHistory(data || []);
    } catch (error) {
  ...
  const loadLayoutPreference = async () => {
    try {
      const response = await fetch(`${BRIDGE_API}/system/settings?key=roulette-swap-layout`);
      const data = await response.json();
      if (data.value === 'true') {
  ...
  const calculateAndSubmitResult = async (gameId, finalBallAngle, finalWheelRotation) => {
    ...
    try {
      const response = await fetch(`${BRIDGE_API}/roulette/result`, {
        method: 'POST',
  ...
  const handleRouletteResult = (data) => {
    ...
    hideTimeout = setTimeout(async () => {
      ...
      try {
        ...
        const response = await fetch(`${BRIDGE_API}/roulette/start?channel=${channel()}`);
