...
async fn handle_get_all(query: String) -> Response<BoxBody<Bytes, Infallible>> {
    ...
    let offset = parse_query_param(&query, "offset")
        .and_then(|s| s.parse::<i64>().ok())
        .unwrap_or(0);
    let order_by = parse_query_param(&query, "order_by")
        .unwrap_or_else(|| "watchtime".to_string());
    ...
    match rusqlite::Connection::open(&conn) {
        Ok(conn) => {
            ...
            let order_clause = match order_by.as_str() {
                ...
                "last_seen" => "ORDER BY last_seen DESC",
                _ => "ORDER BY total_minutes DESC", // Default: watchtime
            };
