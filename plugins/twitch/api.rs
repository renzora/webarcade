...
impl TwitchApiClient {
    ...
    pub async fn search_categories(&self, query: &str) -> Result<serde_json::Value> {
        let client = Client::new();
        let encoded_query = urlencoding::encode(query);
        let url = format!("{}/search/categories?query={}", self.api_base_url, encoded_query);

        log::info!("[Twitch API] Searching categories: {}", query);
        ...
        if !response.status().is_success() {
            let error_text = response.text().await?;
            anyhow::bail!("Failed to search categories: {}", error_text);
        }
